...
export default function Edit( { attributes, setAttributes } ) {
    // ── Preview mode (Block Inserter thumbnail) ─────────────────────────────
    ...
    if ( isPreview ) {
        return (
            <div style={ { background: '#1a1a1a', padding: '4rem 2rem', textAlign: 'center', color: '#fff' } }>
                <span style={ { display: 'block', fontSize: '0.75rem', letterSpacing: '0.3em', opacity: 0.5, marginBottom: '0.75rem', textTransform: 'uppercase' } }>
                    { attributes.sectionBadge }
                </span>
                <h2 style={ { fontSize: '2rem', fontWeight: 400, margin: '0 0 2rem' } }>{ attributes.sectionTitle }</h2>
                <div style={ { display: 'flex', gap: '1rem', overflow: 'hidden' } }>
                    { [ 1, 2, 3 ].map( ( i ) => (
                        <div key={ i } style={ { flex: '0 0 31%', aspectRatio: '4/3', background: '#333', borderRadius: '4px', position: 'relative' } }>
                            <div style={ { position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.5rem', background: 'linear-gradient(transparent,rgba(0,0,0,.8))' } }>
                                <p style={ { color: '#fff', margin: 0, fontSize: '0.9rem' } }>Dự Án { i }</p>
                            </div>
                        </div>
                    ) ) }
                </div>
